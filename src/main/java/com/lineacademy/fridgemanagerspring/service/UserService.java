...
public class UserService {
    ...
    @Transactional  // 진행 중간에 에러가 만약 발생되면 DB 작업한 것을 롤백 하는 어노테이션
    public User createUser(CreateUserRequest request) {
        // 우리가 Express를 사용할 때에는 schema.prisma만 만들고
        // generate를 실행하면 우리가 사용할 수 있는 모든 메서드가 작성되어서 만들어지고 사용만 했으면 됨

        // Spring-Boot, Hibernate에서는 메서드를 자동으로 만들어주는건 findById (primaryKey를 대상으로 검색)만 있고
       // 나머지는 직접 만들어야 함. 대신 '메서드명'을 작성하면 해당 코드는 알아서 만들어줌

       // 이러한 구조에서 데이터베이스 등의 저장소에 접근하는 기능을 모음 클래스들을 "레포지토리(repository)"라고 함

        // 이메일 중복 체크
        if (userRepository.findByEmail(request.getEmail()).isPresent()) {
            throw new IllegalArgumentException("ALEADY_EXISTS_EMAIL");
        }

        // 닉네임 중복 체크
        if (userRepository.existsByNickname(request.getNickname())) {
            throw new IllegalArgumentException("ALEADY_EXISTS_NICKNAME");
